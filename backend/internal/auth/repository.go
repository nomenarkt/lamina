...
type Repository interface {
	CreateUserInvite(ctx context.Context, email, userType string, accessExpires *time.Time) (int64, error)
...
func (r *repositoryImpl) CreateUserInvite(ctx context.Context, email, userType string, accessExpires *time.Time) (int64, error) {
	query := `
		INSERT INTO users (email, user_type, role, status, access_expires_at, created_at)
		VALUES ($1, $2, 'user', 'pending', $3, now())
		RETURNING id
	`
	var id int64
	err := r.db.QueryRowContext(ctx, query, email, userType, accessExpires).Scan(&id)
	return id, err
}
